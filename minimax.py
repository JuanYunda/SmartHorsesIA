...
def minimax(mapa,profundidadMaxima,nodoHorse):
    ...
    mapaActual = copy.deepcopy(nodo.getMapa())
    nodo = Horse(horse1X,horse1Y,horse2X,horse2Y,0,0,mapaActual,None,0,"IA","MAX")
    nodo.setHeuristica()
    colaDeNodos = deque()
    colaMinimax = deque()
    ...
    while (True):
        ...
        if nodo.getMinMax() == "MAX":
            ...
            if(nodo.getPosXIA() - 2 >= 0 and nodo.getPosYIA() - 1 >= 0):
                ...
                nuevoNodo = Horse(nodo.getPosXIA(),nodo.getPosYIA(),nodo.getPosXJUG(),nodo.getPosYJUG(), nodo.getPuntosJUG(), nodo.getPuntosIA(), copiaMapa,nodo,nodo.getProfundidad()+1,"Jugador","MIN")
        ...
        if nodo.getMinMax() == "MIN":
            ...
            if(nodo.getPosXJUG() - 2 >= 0 and nodo.getPosYJUG() - 1 >= 0):
                copiaMapa = copy.deepcopy(nodo.getMapa())
                nuevoNodo = Horse(nodo.getPosXIA(),nodo.getPosYIA(),nodo.getPosXJUG(),nodo.getPosYJUG(),nodo.getPuntosJUG(), nodo.getPuntosIA(),copiaMapa,nodo,nodo.getProfundidad()+1,"IA","MAX")
                nuevoNodo.setHeuristica()
                nuevoNodo.movimiento1()
                colaDeNodos.append(nuevoNodo)
                colaMinimax.append(nuevoNodo)
    ...
    while (True):
        ...
        if nodo.getPadre().getTipo() == "MAX":
            if nodo.getPadre().getUtilidad == 0:
                nodo.getPadre().setUtilidadManual(nodo.getUtilidad())
        ...
        if nodo.getPadre().getTipo() == "MIN":
            if nodo.getPadre().getUtilidad == 0:
                ...
                nodo.getPadre().setUtilidadManual(nodo.getUtilidad())
                MAX = nodo.getUtilidad()

        nodo = colaMinimax.popleft()
        colaRecorrida.append(nodo)
    
    #Recorro la colaRecorrida para encontrar la solucion
    colaFinal = sorted(colaRecorrida, key = nodo.getProfundidad())
    colaResultado = deque()
    mejor = 0
    nodo = colaFinal.popleft()
    while (True):
        profundidad = nodo.getProfundidad()
        if profundidad == 0: 
            colaResultado.append(nodo)
            break

        while (nodo.getProfundidad() == profundidad):
            if nodo.getTipo() == "MAX":
                if nodo.getUtilidad() > mejor:
                    mejor = nodo.getUtilidad
